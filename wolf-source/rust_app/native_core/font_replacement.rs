...
pub(crate) fn replace_complete_font_reference_text(
    ...
    for old_font_name in old_font_names {
        if stripped_text == old_font_name {
            let replacement_reference_name =
                replacement_name_for_old_reference(old_font_name, replacement_font_name);
            return Some(format!(
                "{leading_text}{replacement_reference_name}{trailing_text}"
            ));
        }
...
fn replacement_name_for_old_reference<'a>(
    old_font_name: &str,
    replacement_font_name: &'a str,
) -> String {
    if has_supported_font_suffix(old_font_name) {
        return replacement_font_name.to_string();
    }
    strip_supported_font_suffix(replacement_font_name).to_string()
}

fn has_supported_font_suffix(font_name: &str) -> bool {
    strip_supported_font_suffix(font_name).len() != font_name.len()
}

fn strip_supported_font_suffix(font_name: &str) -> &str {
    let lower_name = font_name.to_ascii_lowercase();
    for suffix in [".woff2", ".woff", ".ttf", ".otf"] {
        if lower_name.ends_with(suffix) {
            return &font_name[..font_name.len() - suffix.len()];
        }
    }
    font_name
}
