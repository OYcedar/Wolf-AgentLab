...
def test_add_game_defaults_source_language_to_japanese() -> None:
    """Wolf 注册默认按日文源文处理，同时仍允许显式覆盖。"""
    parser = build_parser()

    default_args = parser.parse_args(["add-game", "--path", "demo", "--json"])
    assert namespace_optional_str(default_args, "source_language") == "ja"
