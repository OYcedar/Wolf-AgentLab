...
def resolve_registered_game_layout(game_path: str | Path) -> GameLayout:
    """Resolve either an RPG Maker MV/MZ layout or a Wolf layout."""
    resolved_game_path = resolve_game_directory(game_path)
    try:
        return resolve_game_layout(resolved_game_path)
    except FileNotFoundError:
        if is_wolf_game_directory(resolved_game_path):
            return resolve_wolf_game_layout(resolved_game_path)
        raise


def read_registered_game_title(game_path: str | Path) -> str:
    """Read a title for either an RPG Maker MV/MZ game or a Wolf game."""
    resolved_game_path = resolve_game_directory(game_path)
    try:
        return read_game_title(resolved_game_path)
    except FileNotFoundError:
        if is_wolf_game_directory(resolved_game_path):
            return read_wolf_game_title(resolved_game_path)
        raise
...
async def read_metadata(connection: aiosqlite.Connection, db_path: Path) -> GameMetadata:
    ...
    if engine_kind_text not in {"mv", "mz", "wolf"}:
        raise RuntimeError(f"metadata.engine_kind 非法，请重新注册游戏: {db_path}")
    engine_kind: EngineKind = cast(EngineKind, engine_kind_text)
...
class GameRegistry:
    ...
    async def register_game(
        ...
        _ = ensure_db_directory(self.db_directory)
        resolved_game_path = resolve_game_directory(game_path)
        layout = resolve_registered_game_layout(resolved_game_path)
        game_title = read_registered_game_title(resolved_game_path)
    ...
    async def resolve_registered_title_by_path(self, game_path: str | Path) -> str:
        ...
        title = read_registered_game_title(resolved_game_path)
...
__all__: list[str] = [
    ...
    "ensure_db_directory",
    "read_registered_game_title",
    "resolve_default_db_directory",
    "resolve_registered_game_layout",
]
