...
from app.persistence import RuleReviewStateRecord, TargetGameSession
...
from app.wolf.dec import require_prepared_wolf_game
...
class DoctorAgentMixin:
    ...
    async def _check_wolf_game(
        ...
        *,
        ...
        """Check Wolf toolchain and prepared dump state without RPG Maker-only loaders."""
        setting = load_setting(self.setting_path, source_language=session.source_language)
        ...
        summary["source_language"] = session.source_language
        summary["target_language"] = session.target_language
        summary["game_path"] = str(session.game_path)
        summary["engine_version"] = session.engine_version
        archive_paths = iter_wolf_archives(session.game_path)
        summary["wolf_archive_count"] = len(archive_paths)
        if is_wolf_engine_version_unsupported(session.engine_version):
            warnings.append(
                issue(
                    "wolf_engine_version_risky",
                    f"Wolf 引擎版本 {session.engine_version} 不低于 3.595，WolfTL/解包兼容性风险较高",
                )
            )
        if archive_paths:
            warnings.append(
                issue(
                    ...
                    f"游戏目录下仍有 {len(archive_paths)} 个 .wolf 包；写回前请移动到游戏目录外，避免游戏继续读取封包",
                )
            )

        if tools.wolftl is None:
            if not _has_issue(errors, "wolftl_missing"):
                errors.append(issue("wolftl_missing", "WolfTL.exe 未找到；请放入 G:\\wolf\\tools 或在 setting.toml / ATT_WOLF_WOLFTL_PATH 中配置"))
        else:
            _append_check(details, "wolftl", "ok")
        if tools.uberwolf is None:
            if not _has_issue(warnings, "uberwolf_missing"):
                warnings.append(issue("uberwolf_missing", "UberWolf.exe 未找到；prepare-game 会直接尝试 WolfDec fallback"))
        else:
            _append_check(details, "uberwolf", "ok")
        if tools.wolfdec is None:
            if tools.uberwolf is None:
                if not _has_issue(errors, "wolf_unpacker_missing"):
                    errors.append(issue("wolf_unpacker_missing", "UberWolf.exe 和 WolfDec.exe 都未找到，无法解包 .wolf 游戏"))
            else:
                if not _has_issue(warnings, "wolfdec_missing"):
        ...
        try:
            ...
            warnings.append(issue("wolf_not_prepared", f"Wolf 工作区尚未准备或已失效，请运行 prepare-game：{type(error).__name__}: {error}"))
            summary["prepared"] = False
            return

        summary["prepared"] = True
        summary["prepared_data_dir"] = str(prepared.data_dir) if prepared.data_dir is not None else ""
        summary["dump_dir"] = str(prepared.dump_dir) if prepared.dump_dir is not None else ""
        if prepared.dump_dir is None:
            warnings.append(issue("wolf_dump_missing", "prepare 报告缺少 WolfTL dump 目录"))
            return
        ...
        item_count = sum(len(data.translation_items) for data in data_map.values())
        translated_items = await session.read_translated_items()
        runtime_issues = audit_wolf_runtime_translations(
            dump_dir=prepared.dump_dir,
            translated_items=translated_items,
        )
        summary["wolf_text_item_count"] = item_count
        summary["translated_count"] = len(translated_items)
        summary["runtime_issue_count"] = len(runtime_issues)
        if runtime_issues:
            errors.append(issue("runtime_sensitive_translation", f"发现 {len(runtime_issues)} 个 Wolf 运行敏感译文风险"))
    ...
    def _check_static_paths(
        ...
        *,
        ...
        try:
            ...
            if tools.wolftl is None:
                errors.append(issue("wolftl_missing", "WolfTL.exe 未找到；请放入 G:\\wolf\\tools 或配置 setting.toml / ATT_WOLF_WOLFTL_PATH"))
            if tools.uberwolf is None:
                warnings.append(issue("uberwolf_missing", "UberWolf.exe 未找到；prepare-game 会使用 WolfDec fallback"))
            if tools.wolfdec is None:
                if tools.uberwolf is None:
                    errors.append(issue("wolf_unpacker_missing", "UberWolf.exe 和 WolfDec.exe 都未找到，无法解包 .wolf 游戏"))
                else:
...
def _has_issue(items: list[AgentIssue], code: str) -> bool:
