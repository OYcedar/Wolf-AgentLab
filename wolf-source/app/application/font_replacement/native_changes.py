...
FONT_FILE_SUFFIXES = (".woff2", ".woff", ".ttf", ".otf")
...
def apply_native_font_change(
    *,
    ...
    replaced_text = normalize_replaced_font_reference(
...
def normalize_replaced_font_reference(
    *,
    original_text: str,
    replaced_text: str,
    replacement_font_name: str,
) -> str:
    """裸字体族引用替换成目标字体裸名，文件引用保留完整文件名。"""
    original_reference = original_text.strip()
    replaced_reference = replaced_text.strip()
    if not original_reference or not replaced_reference:
        return replaced_text
    original_leaf = extract_font_reference_leaf(original_reference)
    replaced_leaf = extract_font_reference_leaf(replaced_reference)
    if has_font_suffix(original_leaf) or not has_font_suffix(replaced_leaf):
        return replaced_text
    replacement_stem = strip_font_suffix(replacement_font_name)
    prefix_length = len(replaced_text) - len(replaced_text.lstrip())
    suffix_start = len(replaced_text.rstrip())
    leading_text = replaced_text[:prefix_length]
    trailing_text = replaced_text[suffix_start:]
    body = replaced_text[prefix_length:suffix_start]
    separator_index = max(body.rfind("/"), body.rfind("\\"))
    if separator_index >= 0:
        body = f"{body[:separator_index + 1]}{replacement_stem}"
    else:
        body = replacement_stem
    return f"{leading_text}{body}{trailing_text}"

def extract_font_reference_leaf(text: str) -> str:
    """取字体引用末尾文件名或字体族名。"""
...
def has_font_suffix(font_name: str) -> bool:
    """判断字体引用是否已经是文件名。"""
    return any(font_name.lower().endswith(suffix) for suffix in FONT_FILE_SUFFIXES)

def strip_font_suffix(font_name: str) -> str:
    """去掉目标字体文件名扩展名，得到字体族裸名。"""
