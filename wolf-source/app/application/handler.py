...
class TranslationHandler:
    ...
    def _load_wolf_text_rules(
        self,
        setting: Setting,
        custom_placeholder_rules_text: str | None = None,
        placeholder_rule_records: list[PlaceholderRuleRecord] | None = None,
    ) -> TextRules:
        """Load text rules with built-in Wolf control-code protections."""
        if custom_placeholder_rules_text is not None:
            custom_rules = load_custom_placeholder_rules_text(custom_placeholder_rules_text)
        elif placeholder_rule_records is not None:
            custom_rules = tuple(
    ...
    async def add_game(
        ...
        if record.engine_kind != "wolf":
            _ = await load_game_data(record.game_path, include_plugin_source_files=False)
    ...
    async def _translate_wolf_text_in_session(
        self,
        *,
        session: TargetGameSession,
        setting: Setting,
        ...
        """Translate text extracted from a prepared WolfTL dump."""
        set_progress, advance_progress, set_status = callbacks
        ...
        translation_data_map = extract_wolf_translation_data_map(
        ...
        if total_extracted_items == 0:
            return TextTranslationSummary(
                ...
                success_count=0,
                error_count=0,
                blocked_reason="WolfTL dump did not contain safe translatable display text.",
            )
        if pending_count == 0:
            set_progress(total_extracted_items, total_extracted_items)
            return TextTranslationSummary(
        ...
        if not batches:
            return TextTranslationSummary(
                ...
                success_count=0,
                error_count=0,
                blocked_reason="Wolf translation data produced no model batches after deduplication.",
            )
        ...
        if runtime_issues:
            ...
            return TextTranslationSummary(
                ...
                batch_count=0,
                success_count=0,
                error_count=len(runtime_issues),
                blocked_reason=f"Runtime-sensitive translation exists: {first.location_path}",
            )
        run_record = await session.start_translation_run(
            total_extracted=total_extracted_items,
            pending_count=pending_count,
        ...
        set_status(f"Wolf dump pending {pending_count}, deduplicated {deduplicated_count}, batches {len(batches)}")
        source_residual_rule_set = SourceResidualRuleSet.from_records(
            await session.read_source_residual_rules()
        )
        text_translation = TextTranslation(
        ...
        try:
            ...
            finished_run = run_record.model_copy(
                update={
                    ...
                    "quality_error_count": error_count,
                    "finished_at": current_timestamp_text(),
                    "stop_reason": "" if error_count == 0 else "Wolf translation quality errors",
                    "last_error": "" if error_count == 0 else "quality_errors",
                }
            )
            await session.write_translation_run(finished_run)
    ...
    async def write_back(
        ...
        async with await self.game_registry.open_game(game_title) as session:
            ...
            if session.engine_kind == "wolf":
                _ = advance_progress
                prepared = require_prepared_wolf_game(session.game_title)
                if prepared.dump_dir is None or prepared.data_dir is None:
                    raise RuntimeError("Prepared Wolf workspace is incomplete")
                ...
                if tools.wolftl is None:
                    raise FileNotFoundError("WolfTL.exe not found")
                translated_items = await session.read_translated_items()
                runtime_issues = audit_wolf_runtime_translations(
                    dump_dir=prepared.dump_dir,
                ...
                if runtime_issues:
                    first = runtime_issues[0]
                    raise RuntimeError(f"Runtime-sensitive Wolf translation blocked write-back: {first.location_path}")
                wolf_archives = iter_wolf_archives(session.game_path)
                if wolf_archives:
                    first_archive = wolf_archives[0]
                    raise RuntimeError(
                ...
                target_data_dir = session.game_path / "Data"
                backup_stamp = current_timestamp_text().replace(":", "").replace("-", "").replace("T", "_")
                backup_dir = session.game_path / f"Data.att-wolf-backup-{backup_stamp}"
                ...
                return WriteBackSummary(
                    ...
                    source_font_count=0,
                    replaced_font_reference_count=result.restored_runtime_string_count,
                    font_copied=False,
                )
            game_data = await self._load_session_game_data(session)
            setting = self._load_setting(
