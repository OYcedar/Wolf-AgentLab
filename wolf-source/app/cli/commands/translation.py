...
from app.cli.commands.wolf import run_wolf_quality_report_command, run_wolf_verify_feedback_text_command
...
async def run_quality_report_command(args: argparse.Namespace) -> int:
    ...
    async with await GameRegistry().open_game(game_title) as session:
        if session.engine_kind == "wolf":
            return await run_wolf_quality_report_command(args)
...
async def run_verify_feedback_text_command(args: argparse.Namespace) -> int:
    ...
    async with await GameRegistry().open_game(game_title) as session:
        if session.engine_kind == "wolf":
            return await run_wolf_verify_feedback_text_command(args)
...
async def run_translation_status_command(args: argparse.Namespace) -> int:
    ...
    async with await GameRegistry().open_game(game_title) as session:
        if session.engine_kind == "wolf":
            report = await WolfService().translation_status(game_title=game_title)
            write_report_outputs(report=report, args=args, title="Wolf 正文翻译状态")
            return 1 if report.status == "error" else 0
    service = AgentToolkitService()
