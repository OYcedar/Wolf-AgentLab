...
def build_parser() -> argparse.ArgumentParser:
    """构建项目主命令行解析器。"""
    parser = CliArgumentParser(prog="att-wolf", description="Wolf RPG Editor 翻译工具命令行入口")
    ...
    add_game_parser = subparsers.add_parser("add-game", help="注册新的 Wolf 游戏目录")
    _ = add_game_parser.add_argument("--path", required=True, help="Wolf 游戏根目录")
    _ = add_game_parser.add_argument(
        "--source-language",
        choices=["ja", "en"],
        default="ja",
        help="游戏原文语言，默认 ja 表示日文；en 表示英文",
    )
    _ = add_game_parser.add_argument("--json", action="store_true", dest="json_output", help="输出机器可读 JSON")

    prepare_game_parser = subparsers.add_parser("prepare-game", help="解包并验证 Wolf 游戏 Data")
    add_optional_target_arguments(prepare_game_parser)
    _ = prepare_game_parser.add_argument("--json", action="store_true", dest="json_output", help="输出机器可读 JSON")

    dump_text_parser = subparsers.add_parser("dump-text", help="使用 WolfTL 解析已准备的 Wolf 文本")
    add_optional_target_arguments(dump_text_parser)
    _ = dump_text_parser.add_argument("--json", action="store_true", dest="json_output", help="输出机器可读 JSON")
    ...
    audit_runtime_parser = subparsers.add_parser("audit-runtime", help="检查 Wolf 运行敏感字符串是否被翻译破坏")
    add_optional_target_arguments(audit_runtime_parser)
    _ = audit_runtime_parser.add_argument("--json", action="store_true", dest="json_output", help="输出机器可读 JSON")

    restore_runtime_parser = subparsers.add_parser("restore-runtime", help="从原始 dump 恢复 Wolf 运行敏感字符串")
    add_optional_target_arguments(restore_runtime_parser)
    _ = restore_runtime_parser.add_argument("--json", action="store_true", dest="json_output", help="输出机器可读 JSON")

    make_patch_parser = subparsers.add_parser("make-patch", help="生成独立 Wolf 汉化补丁目录")
    add_optional_target_arguments(make_patch_parser)
    _ = make_patch_parser.add_argument("--json", action="store_true", dest="json_output", help="输出机器可读 JSON")
    ...
    command_names = frozenset(subparsers.choices)
    setattr(parser, "_att_wolf_command_names", command_names)
    setattr(parser, "_att_mz_command_names", command_names)
    return parser
...
def parser_command_names(parser: argparse.ArgumentParser) -> frozenset[str]:
    ...
    raw_value = getattr(parser, "_att_wolf_command_names", None)
